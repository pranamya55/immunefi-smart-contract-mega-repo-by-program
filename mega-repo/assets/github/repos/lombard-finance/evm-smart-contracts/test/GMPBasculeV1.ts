...
import { GMPBasculeV1, IGMPBascule } from '../typechain-types';
...
describe('GMPBasculeV1', function () {
  let _: Signer,
    owner: Signer,
    pauser: Signer,
    mintReporter: Signer,
  ...
  let bascule: GMPBasculeV1 & Addressable;
  ...
  before(async function () {
    ...
    bascule = await deployContract<GMPBasculeV1 & Addressable>(
      'GMPBasculeV1',
      [
        owner.address,
        pauser.address,
  ...
  describe('Report and validate mints', function () {
    describe('Main flow', function () {
      ...
      it('Report mint', async function () {
        expect((await bascule.mintHistory(mintID))[1]).to.be.eq(0);
        ...
        expect((await bascule.mintHistory(mintID))[1]).to.be.eq(1);
      });
      ...
      it('Validate mint', async function () {
        ...
        expect((await bascule.mintHistory(mintID))[1]).to.be.eq(2);
      });
