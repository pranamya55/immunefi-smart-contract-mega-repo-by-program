...
describe("Change asset's symbol to a new one by 3 parties", function () {
	...
	it('Bob offers another name for the asset', async () => {
		...
		expect(vars['by_largest_a2s_' + this.asset]).to.be.equal(this.symbol1)
	...
	it('Charlie supports Bob', async () => {
		...
		const amount = 0.2e9
		const drawer_key = this.charlieAddress + '_' + 0 + '_' + symbol + '_' + this.asset
