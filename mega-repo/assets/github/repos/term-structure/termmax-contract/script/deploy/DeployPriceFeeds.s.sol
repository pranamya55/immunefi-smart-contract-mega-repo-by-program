...
contract DeployPriceFeeds is Script {
    function run() public {
...

        {
            ...
            address pt_market = 0xEbF7FD1ec45F505175D92dB4D180B8f323C17875;
            // chainlink(sUSDe/USD): https://data.chain.link/feeds/ethereum/mainnet/susde-usd
            // heartBeat: 86400 (24hr)
            address tusdeToUsd = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419;
            // new price feed heartBeat = max(86400) = 86400 (24hr)
            AggregatorV3Interface ptFeed = AggregatorV3Interface(
                priceFeedFactory.createPTWithPriceFeed(pendlePYLpOracle, pt_market, 900, tusdeToUsd)
            );
            (, int256 answer,,,) = ptFeed.latestRoundData();
            console.log("pt_tusde_29MAY2025 price feed address", address(ptFeed));
            console.log("pt_tusde_29MAY2025 last answer", answer);
        }
