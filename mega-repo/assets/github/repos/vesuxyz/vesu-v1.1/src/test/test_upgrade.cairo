...
mod TestUpgrade {
    ...
    #[test]
    #[fork("Mainnet")]
    fn test_upgrade_claim_fees_integration() {
        let pool_id = 0x052fb52363939c3aa848f8f4ac28f0a51379f8d1b971d8444de25fbd77d8f161;
        ...
        // let singleton_v2_class_hash =
        // 0x053954317aa06c4ba84c54245e85d1a152a7b4a0f4463e5da393cb8e2d31b21e.try_into().unwrap();
        // let extension_v2_class_hash =
        // 0x075dc76272d8af04bb93a363ff0706aa1b5d876d9fb877532c5d4ad7d5c49a6e.try_into().unwrap();

        // let singleton_v2_class_hash =
        // 0x62c6e1d7bd892e63a3514c672e9cfe63bfb647fac4eccc54517f135c25750ee.try_into().unwrap();
        // let extension_v2_class_hash =
        // 0x265473f45bdbbf68262c298fdd6e2656cabc4046c82ebd389256eab2a530036.try_into().unwrap();
    ...
    fn test_upgrade() {
        ...
        while i < pool_ids.len() {
            ...
            while j < assets_copy.len() {
                ...
                // fee share calculation changed, need to reset unclaimed fee shares to 0
                singleton.upgrade(singleton_v2_class_hash);
                singleton.claim_fee_shares(pool_id, collateral_asset);

                singleton.upgrade(singleton_v1_class_hash);
                // return 0 for last_updated in v1 too
    ...
    fn test_upgrade_integration() {
        ...
        let user = contract_address_const::<0x03976cac265a12609934089004df458ea29c776d77da423c96dc761d09d24124>();
        let amount = 1000 * 1000000;

        start_cheat_caller_address(usdc.contract_address, user);
        usdc.approve(singleton.contract_address, amount);
        stop_cheat_caller_address(usdc.contract_address);

        let balance_before = usdc.balance_of(user);
        ...
        assert!(usdc.balance_of(user) == balance_before - amount);
    }
}
