...
import {GroupedClipBreakerFactory} from "src/clip-breaker/GroupedClipBreakerSpell.sol";

contract GroupedClipBreakerDeployScript is Script {
    using stdJson for string;
    using ScriptTools for string;

    string constant NAME = "grouped-clip-breaker-deploy";
    string config;

    GroupedClipBreakerFactory fab;
    string[] ilkStrs;

    function run() external {
        config = ScriptTools.loadConfig();

        fab = GroupedClipBreakerFactory(config.readAddress(".factory", "FOUNDRY_FACTORY"));
