...
contract Vat {
    ...
    // --- Logs ---
    event LogNote(
        bytes4 indexed sig, bytes32 indexed arg1, bytes32 indexed arg2, bytes32 indexed arg3, bytes data
    ) anonymous;
    ...
    function file(bytes32 ilk, bytes32 what, uint256 data) external note {
    ...
    function grab(bytes32 i, address u, address v, address w, int256 dink, int256 dart) external note {
    ...
    function suck(address u, address v, uint256 rad) external note {
        sin[u] = add(sin[u], rad);
        dai[v] = add(dai[v], rad);
        vice = add(vice, rad);
        debt = add(debt, rad);
    ...
    // --- Rates ---
    function fold(bytes32 i, address u, int256 rate) external note {
        Ilk storage ilk = ilks[i];
