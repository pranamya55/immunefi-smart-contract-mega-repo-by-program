...
export async function preSetup(provider: JsonRpcProvider): Promise<void> {
  ...
  try {
    // T2 [TYPE 2] VaultT2_DEX-ETH-osETH_wstETH - 159
    {
      const vaultType = "T2";
      const supplyToken = "0xC0652bdDcfF7739dadf0C9567584B35Ca63eB8E1"; // ETH-osETH
      const borrowToken = "0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0"; // wstETH

      await provider.send("eth_sendTransaction", [
        {
          from: "0x4F6F977aCDD1177DCD81aB83074855EcB9C2D49e",
          to: "0x324c5Dc1fC42c7a4D43d92df1eBA58a54d13Bf2d",
          data: getDeployVaultCalldata(
            vaultType,
            VAULT_LOGIC_ADDRESS(vaultType),
            supplyToken,
            borrowToken
          ),
          value: "",
...
const getDeployVaultCalldata = (
  ...
  const logicData = new ethers.Interface(ABI).encodeFunctionData(
    `vault${vaultType}`,
    [supplyToken, borrowToken]
  );
  const deployerData = new ethers.Interface(DEPLOYERABI).encodeFunctionData(
    "deployVault",
    [logic, logicData]
  );
  return deployerData;
