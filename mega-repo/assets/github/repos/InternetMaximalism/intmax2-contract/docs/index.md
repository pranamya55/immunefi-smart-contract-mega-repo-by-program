...
### migrateStartTimestamp

```solidity
function migrateStartTimestamp(uint256 _startTimestamp) external
```

### migrateNullifiers

```solidity
function migrateNullifiers(bytes32[] _nullifiers) external
```

### migrateContributions

```solidity
function migrateContributions(uint256[] periodNumbers, address[] users, uint256[] depositAmounts) external
```

### migrateConsumeUserAllocation

```solidity
function migrateConsumeUserAllocation(uint256[] periodNumbers, address[] users) external
```

### migrateNullifierNonce

```solidity
function migrateNullifierNonce(uint256 _nullifierNonce) external
```
...
```solidity
function setCircuitDigest(uint256 _circuitDigest) external
```
...
### CircuitDigestUpdated

```solidity
event CircuitDigestUpdated(uint256 circuitDigest)
```

Emitted when circuit digest is updated
...
```solidity
function migrateContribution(struct AllocationLib.State state, uint256 period, address user, uint256 depositAmount) internal
```
...
```solidity
function getHash(struct ClaimProofPublicInputsLib.ClaimProofPublicInputs inputs) internal pure returns (uint256)
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| [0] | uint256 | bytes32 The resulting hash that is masked to fit within 253 bits |
...
Interface for migration functionality in contracts

_Defines the functions and events for contract migration_
...
Error when trying to migrate already migrated contract
...
Emitted when the contract is migrated to a new version
...
Error thrown when native token transfer fails

#### Parameters

| Name | Type | Description |
| ---- | ---- | ----------- |
| recipient | address | The address that was supposed to receive the native tokens |
| amount | uint256 | The amount of native tokens that were attempted to be transferred |
...
Event emitted when the rollup contract is updated

#### Parameters

| Name | Type | Description |
| ---- | ---- | ----------- |
| rollup | address | The address of the new rollup contract |
...
```solidity
function updateRollupContract(address _rollup) external
```
...
```solidity
function migrateWithdrawals(struct WithdrawalLib.Withdrawal[] withdrawals) external
```
...
```solidity
function getHash(struct WithdrawalProofPublicInputsLib.WithdrawalProofPublicInputs inputs) internal pure returns (uint256)
```
...
### migrateLastProcessedDepositId

```solidity
function migrateLastProcessedDepositId(uint256 _lastProcessedDepositId) external
```

### migrateDeposits

```solidity
function migrateDeposits(bytes32[] _depositHashes) external
```

### migrateBlockPost

```solidity
function migrateBlockPost(bytes32 _prevBlockHash, address _sender, uint64 _timestamp, uint32 _blockNumber, bytes32 _depositTreeRoot, bytes32 _signatureHash, bytes _originalCallData) external
```

### migrateBlockBuilderNonce

```solidity
function migrateBlockBuilderNonce(address[] _builder, uint32[] _registrationNonces, uint32[] _nonRegistrationNonces) external
```
