// @source: https://github.com/lidofinance/stonks/blob/551d9b024f8c0ac0ef3d1266d1ac59ce0f238bf0/scripts/verify-bytecode.ts
...
const API_URL = process.env['API_URL'] ?? 'https://api.etherscan.io/api?'
...
async function main() {
  console.log(`Verifying the contract ${chalk.yellow(CONTRACT)} over ${chalk.yellow(API_URL)}\n`)

  console.log(`Downloading the source code from the Etherscan...`)
  const code = await loadEtherscanSourceCode(ETHERSCAN_API_KEY, API_URL, CONTRACT)
  ...
  console.log(`Compiling the contracts downloaded from the ${chalk.yellow(API_URL)}...`)
