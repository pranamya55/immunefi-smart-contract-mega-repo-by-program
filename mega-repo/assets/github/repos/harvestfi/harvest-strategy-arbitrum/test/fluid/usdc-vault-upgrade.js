...
const VaultNew = artifacts.require("VaultV2");
const VaultProxy = artifacts.require("VaultProxy");

//const Strategy = artifacts.require("");
const Strategy = artifacts.require("FluidLendStrategyMainnet_USDC");

// Developed and tested at blockNumber 398743600

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Arbitrum Mainnet Fluid Lend USDC - vault upgrade", function() {
  let accounts;
  ...
  before(async function() {
    ...
    await vault.scheduleUpgrade(addresses.VaultImplementationV2New, {from: governance});
    await Utils.waitHours(13);
    await vaultProxy.upgrade({from: governance});

    const before = await vault.previewWithdraw(1);
    console.log("Before upgrade withdraw preview for 1e2:", before.toString());
    vault = await VaultNew.at(vault.address);
    const after = await vault.previewWithdraw(1);
    console.log("After upgrade withdraw preview for 1e2:", after.toString());
