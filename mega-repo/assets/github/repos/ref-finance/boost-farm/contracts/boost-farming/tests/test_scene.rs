...
fn test_normal_seed_single_farm(){
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id, to_yocto("100")).unwrap_json::<bool>());
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id, to_yocto("10")).unwrap_json::<bool>());
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id, free_amount_part).unwrap_json::<bool>());
...
fn test_normal_mutli_farm(){
    ...
    let free_amount = to_yocto("50") - seed_slashed;
    assert_user_seed_info(e.get_farmer_seed(&users.farmer2, &seed_id), free_amount, 0, 0, 0, 0);
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id, free_amount).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer2, &token_id), to_yocto("50") + free_amount);
    e.get_farmer_seed(&users.farmer2, &seed_id).is_null();
    assert_seed(e.get_seed(&seed_id), &seed_id, TOKEN_DECIMALS as u32, 2, to_yocto("50"), to_yocto("100"), MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC);
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id, to_yocto("50")).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id), to_yocto("100"));
    assert_seed(e.get_seed(&seed_id), &seed_id, TOKEN_DECIMALS as u32, 2, 0, 0, MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC);
...
fn test_mutli_seed_with_booster_and_normal(){
    ...
    affected_seeds.insert(seed_id_booster.clone(), U128(10 * 10u128.pow(24)));
    ...
    e.lock_free_seed(&users.farmer1, &booster_id, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC, to_yocto("10"));
    ...
    assert_eq!(e.get_farmer_seed(&users.farmer2, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(null));
    ...
    e.lock_free_seed(&users.farmer2, &booster_id, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC, to_yocto("10"));
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id_normal, to_yocto("100")).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id_normal), to_yocto("100"));
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id_booster, to_yocto("100")).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id_booster), to_yocto("100"));
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id_normal, free_amount).unwrap_json::<bool>());
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id_booster, free_amount).unwrap_json::<bool>());
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id_normal, to_yocto("50")).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id_normal), to_yocto("100"));
    assert_seed(e.get_seed(&seed_id_normal), &seed_id_normal, TOKEN_DECIMALS as u32, 2, 0, 0, MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC);
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id_booster, to_yocto("50")).unwrap_json::<bool>());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id_booster), to_yocto("100"));
    assert_seed(e.get_seed(&seed_id_booster), &seed_id_booster, TOKEN_DECIMALS as u32, 2, 0, 0, MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC);
    assert_farm_detail(e.get_farm(&farm_id_0_booster), to_yocto("300"), e.current_time(), to_yocto("300"), farm_id_0_booster_reward_claim, 0, 0, Some(FarmStatus::Ended));
    assert_farm_detail(e.get_farm(&farm_id_1_booster), to_yocto("150"), e.current_time(), to_yocto("150"), farm_id_1_booster_reward_claim, to_yocto("15"), 0, Some(FarmStatus::Ended));
    ...
    farm_id_0_booster_reward_claim += e.get_unclaimed_rewards(&users.farmer1, &seed_id_booster, &tokens.neth);
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &booster_id, to_yocto("10")).unwrap_json::<bool>());
    ...
    farm_id_0_booster_reward_claim += e.get_unclaimed_rewards(&users.farmer2, &seed_id_booster, &tokens.neth);
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &booster_id, to_yocto("9")).unwrap_json::<bool>());
    assert_eq!(e.ft_balance_of(&tokens.love_ref, &users.farmer2), to_yocto("9"));
    assert_eq!(e.get_farmer_seed(&users.farmer2, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(null));
    assert_seed(e.get_seed(&seed_id_booster), &seed_id_booster, TOKEN_DECIMALS as u32, 2, to_yocto("150"), to_yocto("150"), MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC);
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &booster_id, to_yocto("1")).unwrap_json::<bool>());
...
fn test_booster_seed_mutli_farm(){
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer2, &seed_id, to_yocto("100")).unwrap_json::<bool>());
    assert_eq!(e.ft_balance_of(&tokens.love_ref, &users.farmer2), to_yocto("100"));
    e.get_farmer_seed(&users.farmer2, &seed_id).is_null();
...
fn test_booster_ratio(){
    ...
    let booster_id = tokens.love_ref.account_id.to_string();
    let mut affected_seeds = HashMap::new();
    affected_seeds.insert(seed_id_booster.clone(), U128(10 * 10u128.pow(18)));
    ...
    assert_eq!(e.get_farmer_seed(&users.farmer1, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(null));
    e.ft_stake_free_seed(&users.farmer1, &tokens.love_ref, 5u128 * 10u128.pow(17)).assert_success();
    assert_eq!(e.get_farmer_seed(&users.farmer1, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(null));
    e.ft_stake_free_seed(&users.farmer1, &tokens.love_ref, 1u128 * 10u128.pow(2)).assert_success();
    assert_eq!(e.get_farmer_seed(&users.farmer1, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(null));
    e.lock_free_seed(&users.farmer1, &booster_id, DEFAULT_SEED_MIN_LOCKING_DURATION_SEC, 10u128 * 10u128.pow(17) + 1u128 * 10u128.pow(2)).assert_success();
...
fn test_booster_update(){
    ...
    assert_eq!(e.get_farmer_seed(&users.farmer1, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(2.9999999999999996));
    e.claim_reward_by_seed(&users.farmer1, &seed_id_booster).assert_success();
    assert_eq!(e.get_farmer_seed(&users.farmer1, &seed_id_booster).get("boost_ratios").unwrap()[booster_id.clone()], json!(1.9999999999999996));
    assert_seed(e.get_seed(&booster_id), &booster_id, TOKEN_DECIMALS as u32, 0, to_yocto("10"), to_yocto("1000"), MIN_SEED_DEPOSIT, DEFAULT_SEED_SLASH_RATE, 1);
