...
impl FarmerSeed {
    ...
    pub fn add_lock(&mut self, amount: Balance, duration_sec: u32, config: &Config, seed_min_locking_duration_sec: u32) -> Balance {
        let prev = self.get_seed_power();
        ...
        if self.unlock_timestamp > 0 && self.unlock_timestamp > timestamp {
            ...
            let relocked_x = compute_x_amount(&config, self.locked_amount, duration_sec, seed_min_locking_duration_sec);
            self.x_locked_amount = std::cmp::max(self.x_locked_amount, relocked_x);
            let extra_x = compute_x_amount(config, amount, duration_sec, seed_min_locking_duration_sec);
            self.x_locked_amount += extra_x;
        } else {
            self.x_locked_amount = compute_x_amount(config, self.locked_amount + amount, duration_sec, seed_min_locking_duration_sec);
        }
    ...
    pub fn free_to_lock(&mut self, amount: Balance, duration_sec: u32, config: &Config, seed_min_locking_duration_sec: u32) -> Balance {
        require!(amount <= self.free_amount, E101_INSUFFICIENT_BALANCE);
        let prev = self.get_seed_power();
        self.free_amount -= amount;
    ...
    pub fn try_unlock_all_to_free(&mut self) -> (Balance, Balance) {
        if self.unlock_timestamp < env::block_timestamp() && self.unlock_timestamp != 0 {
            let prev = self.get_seed_power();
            let unlock_amount = self.unlock_all_to_free();
            (unlock_amount, prev - self.get_seed_power())
        } else {
            (0, 0)
        }
    }

    pub fn unlock_all_to_free(&mut self) -> Balance {
        let unlock_amount = self.locked_amount;
        self.free_amount += unlock_amount;
    ...
    // TODO: add comment
    pub fn sync_locking_policy(&mut self, config: &Config, seed_min_locking_duration_sec: u32) {
        let timestamp = env::block_timestamp();
        ...
        let max_x_locked_amount = compute_x_amount(config, self.locked_amount, maximum_locking_duration_sec, seed_min_locking_duration_sec);
        let recalc_duration_sec = std::cmp::max(
            seed_min_locking_duration_sec,
            std::cmp::min(remain_duration_sec, maximum_locking_duration_sec)
        );
        let recalc_x_locked_amount = compute_x_amount(config, self.locked_amount, recalc_duration_sec, seed_min_locking_duration_sec);
...
fn compute_x_amount(config: &Config, amount: u128, duration_sec: u32, seed_min_locking_duration_sec: u32) -> u128 {
    amount
        + u128_ratio(
            amount,
            u128::from(config.max_locking_multiplier - MIN_LOCKING_REWARD_RATIO) * u128::from(to_nano(duration_sec - seed_min_locking_duration_sec)),
            u128::from(to_nano(config.maximum_locking_duration_sec - seed_min_locking_duration_sec)) * MIN_LOCKING_REWARD_RATIO as u128,
        )
}
