...
fn test_lock(){
    ...
    let fifty_percent_booster_duration_sec = (DEFAULT_MAX_LOCKING_DURATION_SEC - DEFAULT_SEED_MIN_LOCKING_DURATION_SEC) / 2 + DEFAULT_SEED_MIN_LOCKING_DURATION_SEC;
    e.mft_stake_lock_seed(&token_id, &users.farmer1, to_yocto("50"), fifty_percent_booster_duration_sec).assert_success();
    assert_user_seed_info(e.get_farmer_seed(&users.farmer1, &seed_id), 0, to_yocto("50"), to_yocto("75"), e.current_time() + to_nano(fifty_percent_booster_duration_sec), fifty_percent_booster_duration_sec);
