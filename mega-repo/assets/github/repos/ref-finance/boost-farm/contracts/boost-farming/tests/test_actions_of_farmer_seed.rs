...
fn test_unlock_and_withdraw_seed(){
    ...
    // 1 : E100_ACC_NOT_REGISTERED
    assert_err!(
        e.unlock_and_withdraw_seed(&users.farmer2, &seed_id, to_yocto("100")),
        E100_ACC_NOT_REGISTERED
    );

    // 2 : E301_SEED_NOT_EXIST
    assert_err!(
        e.unlock_and_withdraw_seed(&users.farmer1, &format!("{}#{}", "seed_id", 1), to_yocto("100")),
        E301_SEED_NOT_EXIST
    );
    ...
    assert_err!( 
        e.unlock_and_withdraw_seed(&users.farmer1, &seed_id, to_yocto("150")),
        E101_INSUFFICIENT_BALANCE
    );
    ...
    assert_err!(
        e.unlock_and_withdraw_seed(&users.farmer1, &seed_id, to_yocto("25")),
    ...
    assert_eq!(true, e.unlock_and_withdraw_seed(&users.farmer1, &seed_id, to_yocto("75")).unwrap_json::<bool>());
    assert!(e.get_farmer_seed(&users.farmer1, &seed_id).is_null());
    assert_eq!(e.mft_balance_of(&users.farmer1, &token_id), to_yocto("75"));
}

#[test]
fn test_unlock_and_withdraw_booster_seed(){
    let e = init_env();
    let users = Users::init(&e);
    ...
    e.modify_booster(&e.owner, &seed_id, &BoosterInfo{
        booster_decimal: 18,
        affected_seeds: Default::default(),
