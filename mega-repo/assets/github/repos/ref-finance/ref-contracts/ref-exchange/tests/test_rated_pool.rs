...
#[test]
fn test_add_liquidity_fails_when_rates_expired() {
    let (root, owner, pool, tokens, token_rated_contracts) =
        setup_rated_pool_with_liquidity(
            vec![near()],
            vec![stnear(), linear()],
    ...
    // Update both token rates to make them valid initially
    call!(
        owner,
        pool.update_token_rate(
            stnear_contract.valid_account_id()
    ...
    // First add liquidity should succeed with valid rates
    let user1 = root.create_user("user1".to_string(), to_yocto("100"));
    mint_and_deposit_token(&user1, &tokens[0], &pool, 100000*ONE_NEAR);
    mint_and_deposit_rated_token(&user1, &token_rated_contracts[0], &pool, 100000*ONE_STNEAR);
    mint_and_deposit_rated_token(&user1, &token_rated_contracts[1], &pool, 100000*ONE_LINEAR);
    ...
    let out_come = call!(
        user1,
        pool.add_stable_liquidity(0, vec![
            U128(100000*ONE_NEAR), U128(100000*ONE_STNEAR), U128(100000*ONE_LINEAR)], U128(1)),
        deposit = to_yocto("0.0007")
    );
    out_come.assert_success();

    // Simulate time passing (25 hours in nanoseconds to exceed the 24-hour expiration)
    root.borrow_runtime_mut().cur_block.block_timestamp += 25 * 3600 * 10u64.pow(9);

    // Now prepare for second add liquidity but with expired rates
    let user2 = root.create_user("user2".to_string(), to_yocto("100"));
    mint_and_deposit_token(&user2, &tokens[0], &pool, 50000*ONE_NEAR);
    mint_and_deposit_rated_token(&user2, &token_rated_contracts[0], &pool, 50000*ONE_STNEAR);
    mint_and_deposit_rated_token(&user2, &token_rated_contracts[1], &pool, 50000*ONE_LINEAR);

    // Try to add liquidity with expired rates (not updated) - should fail
    let out_come = call!(
        user2,
        pool.add_stable_liquidity(0, vec![
            U128(50000*ONE_NEAR), U128(50000*ONE_STNEAR), U128(50000*ONE_LINEAR)], U128(1)),
        deposit = to_yocto("0.0007")
    );
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come).contains("E120: Rates expired"));

    // Update only stnear rate
    call!(
        owner,
        pool.update_token_rate(
            stnear_contract.valid_account_id()
    ...
    // Try again with only one rate updated - should still fail because linear is expired
    let out_come = call!(
        user2,
        pool.add_stable_liquidity(0, vec![
            U128(50000*ONE_NEAR), U128(50000*ONE_STNEAR), U128(50000*ONE_LINEAR)], U128(1)),
    ...
    // Now with both rates updated, add liquidity should succeed
    let out_come = call!(
        user2,
        pool.add_stable_liquidity(0, vec![
            U128(50000*ONE_NEAR), U128(50000*ONE_STNEAR), U128(50000*ONE_LINEAR)], U128(1)),
    ...
    out_come.assert_success();
    assert_eq!(mft_balance_of(&pool, ":0", &user2.account_id()), 150000*ONE_LPT);
}
