...
contract TestGsmFixedPriceStrategy4626 is TestGhoBase {
  ...
  function testOneToOnePriceRatio() public {
    FixedPriceStrategy4626 strategy = new FixedPriceStrategy4626(1e18, address(USDX_4626_TOKEN), 6);
    uint256 usdxIn = 100e6;
    uint256 ghoOut = 100e18;
    assertEq(strategy.getAssetPriceInGho(usdxIn, true), ghoOut, 'Unexpected asset price in GHO');
    assertEq(strategy.getGhoPriceInAsset(ghoOut, false), usdxIn, 'Unexpected gho price in asset');
  }

  function testOneToTwoPriceRatio() public {
    FixedPriceStrategy4626 strategy = new FixedPriceStrategy4626(2e18, address(USDX_4626_TOKEN), 6);
    uint256 usdxIn = 100e6;
  ...
  function testRevertZeroPriceRatio() public {
    ...
    new FixedPriceStrategy4626(0, address(USDX_4626_TOKEN), 6);
  }

  function testPriceFeedHighExchangeRate() public {
    FixedPriceStrategy4626 strategy = new FixedPriceStrategy4626(1e18, address(USDX_4626_TOKEN), 6);
    _mintVaultAssets(USDX_4626_TOKEN, USDX_TOKEN, ALICE, 100e6);

    // Inflate exchange rate to 2
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, 100e6, true);
  ...
  function testPriceFeedLowExchangeRate() public {
    ...
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, 50e6, false);
