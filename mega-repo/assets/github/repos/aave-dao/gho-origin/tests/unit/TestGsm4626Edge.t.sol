...
contract TestGsm4626Edge is TestGhoBase {
  ...
  function testOngoingExposureSellAsset() public {
    ...
    assertEq(GHO_GSM_4626.getAvailableUnderlyingExposure(), DEFAULT_GSM_USDX_EXPOSURE);
    assertEq(GHO_GSM_4626.getAvailableLiquidity(), 0);

    uint128 sellAssetAmount = DEFAULT_GSM_USDX_AMOUNT;
    ...
    assertEq(
      ...
      DEFAULT_GSM_USDX_EXPOSURE - calcExposure
    );
    assertEq(GHO_GSM_4626.getAvailableLiquidity(), calcExposure);
    assertEq(
      USDX_4626_TOKEN.previewRedeem(USDX_4626_TOKEN.balanceOf(address(GHO_GSM_4626))),
      sellAssetAmount
    );

    // Inflate exchange rate
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, sellAssetAmount, true);
    ...
    calcExposure += DEFAULT_GSM_USDX_AMOUNT;
    ghoBought = _sellAsset(GHO_GSM_4626, USDX_4626_TOKEN, USDX_TOKEN, ALICE, sellAssetAmount);
    ...
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, sellAssetAmount * 3, false);
  ...
  function testSellAssetWithHighExchangeRate() public {
    uint256 resultingAssets = DEFAULT_GSM_USDX_AMOUNT * 2;
    ...
    assertEq(USDX_4626_TOKEN.previewRedeem(USDX_4626_TOKEN.balanceOf(ALICE)), resultingAssets);
    ...
    emit SellAsset(ALICE, ALICE, DEFAULT_GSM_USDX_AMOUNT, grossAmount, fee);
  ...
  function testSellAssetWithLowExchangeRate() public {
    ...
    // Deflate exchange rate
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, DEFAULT_GSM_USDX_AMOUNT / 2, false);
    assertEq(USDX_4626_TOKEN.previewRedeem(USDX_4626_TOKEN.balanceOf(ALICE)), resultingAssets);
  ...
  function testExposureLimitWithSharpExchangeRate() public {
    ...
    uint128 depositAmount = DEFAULT_GSM_USDX_EXPOSURE / 2;
    _mintVaultAssets(USDX_4626_TOKEN, USDX_TOKEN, ALICE, depositAmount);

    // Inflate exchange rate
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, depositAmount, true);
    assertEq(
      USDX_4626_TOKEN.previewRedeem(USDX_4626_TOKEN.balanceOf(ALICE)),
      DEFAULT_GSM_USDX_EXPOSURE
    );

    vm.startPrank(ALICE);
    USDX_4626_TOKEN.approve(address(gsm), depositAmount);
    gsm.sellAsset(depositAmount, ALICE);
    assertEq(gsm.getAvailableLiquidity(), depositAmount);
    assertEq(gsm.getAvailableUnderlyingExposure(), DEFAULT_GSM_USDX_EXPOSURE - 1 - depositAmount);
    vm.stopPrank();
  }
  ...
  function testRevertExposureWithSharpExchangeRate() public {
    ...
    // Deflate exchange rate
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, DEFAULT_GSM_USDX_EXPOSURE, false);
    assertEq(
      USDX_4626_TOKEN.previewRedeem(USDX_4626_TOKEN.balanceOf(ALICE)),
      DEFAULT_GSM_USDX_EXPOSURE
    );
  ...
  function testDistributeYieldToTreasury() public {
    ...
    _sellAsset(GHO_GSM_4626, USDX_4626_TOKEN, USDX_TOKEN, ALICE, DEFAULT_GSM_USDX_AMOUNT);
    ...
    uint256 backingBefore = USDX_4626_TOKEN.previewRedeem(
    ...
    uint256 backingAfter = USDX_4626_TOKEN.previewRedeem(
      USDX_4626_TOKEN.balanceOf(address(GHO_GSM_4626))
    );
  ...
  function testDistributeYieldToTreasuryWithExcessExceedingCapacity() public {
    ...
    GHO_RESERVE.setLimit(address(GHO_GSM_4626), capacity);
  ...
  function testGetAccruedFeesWithHighExchangeRateAndMaxedOutCapacity() public {
    ...
    GHO_RESERVE.setLimit(address(GHO_GSM_4626), 0);
  ...
  function testBuyAssetAtCapacityWithGain() public {
    ...
    // Simulate a gain
    _changeExchangeRate(USDX_4626_TOKEN, USDX_TOKEN, DEFAULT_GSM_USDX_EXPOSURE / 4, true);
    ...
    uint128 buyAmount = DEFAULT_CAPACITY / (((5 * DEFAULT_GSM_USDX_EXPOSURE) / 4) / 100);
    ...
    assertEq(USDX_4626_TOKEN.balanceOf(ALICE), buyAmount, 'Unexpected final USDX balance');
    ...
    vm.startPrank(ALICE);
    USDX_4626_TOKEN.approve(address(GHO_GSM_4626), 2);
    vm.expectEmit(true, true, true, true, address(GHO_GSM_4626));
