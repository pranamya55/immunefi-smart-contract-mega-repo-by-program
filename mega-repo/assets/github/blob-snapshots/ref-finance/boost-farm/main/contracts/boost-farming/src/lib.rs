...
impl Config {
    ...
    pub fn get_boosters_from_seed(&self, seed_id: &SeedId) -> Vec<(SeedId, u32, u128, u128)> {
        self.booster_seeds
            .iter()
            .filter(|(k, v)| k.clone() != seed_id && v.affected_seeds.contains_key(seed_id))
            .map(|(k, v)| (k.clone(), v.booster_decimal, v.affected_seeds.get(seed_id).unwrap_or(&U128(0)).0, v.boost_suppress_factor))
            .collect()
    }
}
