...
contract StrategyPolyCatDyfnLP is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public eth = address(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
    address constant public wmaticDfyn = address(0x4c28f48448720e9000907BC2611F73022fdcE1fA);
    address constant public wrapped = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
    address constant public output = address(0x3a3Df212b7AA91Aa0402B9035b098891d276572B);
    address public want;
    ...
    // Routes
    address[] public outputToEthRoute = [output, wmaticDfyn, eth];
    address[] public ethToWrappedRoute = [eth, wrapped];
    ...
    constructor(
        ...
        if (lpToken0 == wmaticDfyn) {
            outputToLp0Route = [output, wmaticDfyn];
        } else if (lpToken0 != output) {
            outputToLp0Route = [output, wmaticDfyn, lpToken0];
        }

        if (lpToken1 == wmaticDfyn) {
            outputToLp1Route = [output, wmaticDfyn];
        } else if (lpToken1 != output) {
            outputToLp1Route = [output, wmaticDfyn, lpToken1];
        }
    ...
    function chargeFees() internal {
        ...
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toEth, 0, outputToEthRoute, address(this), now);
        
        uint256 toWrapped = IERC20(eth).balanceOf(address(this));
        IUniswapRouterETH(quickrouter).swapExactTokensForTokens(toWrapped, 0, ethToWrappedRoute, address(this), now);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(eth).safeApprove(quickrouter, uint256(-1));
    ...
    function _removeAllowances() internal {
        ...
        IERC20(eth).safeApprove(quickrouter, 0);
