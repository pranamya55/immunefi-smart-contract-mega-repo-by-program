...
- Linked via manifest-resolved outputs: 4874
- Linked GitHub blob snapshots: 936
- Linked GitHub repo assets: 153
- Linked GitHub gists: 5
- Unresolved in filtered export: 0

## Resolution Notes

- Added non-EVM recovery for Tronscan and Solana (Alchemy RPC)
- Added EVM bytecode recovery via Etherscan v2 `proxy.eth_getCode`
- Remaining hard failures were materialized as placeholder artifacts so every asset reference is indexed

## Layout

- `<program-slug>/assets.json`: source asset refs for the program
- `<program-slug>/summary.json`: linked counts per program
- `<program-slug>/resolved-by-manifest/`: symlinks keyed by `asset_id`
